...
\begin{Lem}\label{lem:one-sided:supportAndLocalCohomology-}%
    ...
    \begin{enumerate}
        ...
        \item \label{li:lem:one-sided:supportAndLocalCohomology-:2}%
            $H^i(\lc{p}ι_{Z}^*\sheaf F) = 0$ for all $i ≥ -n+1$, all $Z ∈ \measuringFam$ and all $p \in Z$.
    \end{enumerate}
\end{Lem}
...
\begin{proof}
    ...
    Thus by Grothendieck's vanishing theorem \cite[Theorem~6.1.2]{BrodmannSharp:1998:LocalCohomology}, $H^i(\lc{p}ι_Z^*\sheaf F) = 0$ for all $i > \dim ι_Z^*\sheaf F \ge -n$.
    ...
    Pick $Z$ such that $\dim H^0(ι_Z^*\sheaf F) = \dim (\O_Z \otimes_{\O_X} \sheaf F) = \dim Z$.
    For ease of notation, we set $\sheaf G = H^0(ι_Z^*\sheaf F)$.
    Pick $z ∈ Z$ such that $\sheaf G_z$ still has dimension $\dim Z = -k$ [e.g.~take $z$ in an open orbit].

    It suffices to show that $H^{-k}(\lc{z}\sheaf G)_z \ne 0$.
    By flat base change,
    \[
        H^{-k}(\lc{z}\sheaf G)_z = H^{-k}(\lc{z}(\sheaf G_z)).
    \]
    Since $\dim\sheaf G_z = -k$, the Grothendieck non-vanishing theorem implies that the latter is non-zero.
\end{proof}
