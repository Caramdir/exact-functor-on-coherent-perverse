...
To simplify notation, if $x ∈ \Xtop$ is any point, we write $\overline x$ for the closure $\overline{\{x\}}$ and $\dim x = \dim \overline x$.
...
\begin{Prop}\label{prop:equivDeligneKashiwara}%
    ...
    \begin{enumerate}[(a)]
        \item
            ...
            \begin{enumerate}[(i)]
                \item $\sheaf F ∈ \perv D^{≤0}(X)^G$, i.e.\ $\mathbf ι_x^*\sheaf F ∈ D^{≤p(x)}(\catModules{\O_x})$ for all $x ∈ \Xtop$;
                \item $p(\dim \supp H^{k}(\sheaf F)) ≥ k$ for all $k$.
            \end{enumerate}
        \item
            If $p$ is strictly monotone, then the following are equivalent
            \begin{enumerate}[(i)]
                \item $\sheaf F ∈ \perv D^{\ge 0}(X)^G$, i.e.\ $\mathbf ι_x^!\sheaf F ∈ D^{≥p(x)}(\catModules{\O_x})$ for all $x ∈ \Xtop$;
                \item $\lc {\overline x}\sheaf F ∈ D^{≥p(x)}(X)$ for all $x ∈ \Xtop$;
                \item $\lc {Y}\sheaf F ∈ D^{≥p(\dim Y)}(X)$ for all $G$-invariant closed subvarieties $Y$ of $X$;
                \item $\dim\left( \overline x ∩ \supp\left( H^k(\dualize F) \right) \right) ≤ -p(x) - k$ for all $x ∈ \Xtop$ and all $k$.
            \end{enumerate}
    \end{enumerate}
\end{Prop}
...
\begin{Def}\label{def:measuring}%
    ...
    A \emph{$p$-measuring family of subvarieties} is a family $\measuringFam$ of $p$-measuring subvarieties of $X$ such that for each $x ∈ \Xtop$ there exists $Z ∈ \measuringFam$ with $Z ∩ \overline x \ne \emptyset$.
...
\item 
    By Proposition~\ref{prop:equivDeligneKashiwara}(b), $\sheaf F ∈ \perv D^{≥0}(X)^G$ if and only if
    \begin{align}
        \label{eq:main:+supp1}%
