...
For an $\sheaf{O}_X$-module $\sheaf F$ let $\lc Z\sheaf F$ be the subsheaf of $\sheaf F$ of sections with support in $Z$ \cite[Variation~3 in \textsc{iv}.1]{Hartshorne:1966:ResiduesAndDuality}.
...
We further set $\mathbf i_x^!\sheaf F = \mathbf i_x^*\lc {\overline x}$, cf.~\cite[Variation~8 in \textsc{iv}.1]{Hartshorne:1966:ResiduesAndDuality}.
...
\begin{proof}
    ...
    By~\cite[Proposition~\textsc{v}.2.6]{Hartshorne:1966:ResiduesAndDuality}, $\dualize \sheafHom(\sheaf G,\sheaf F) = \sheaf G \otimes_{\sheaf{O}_X} \dualize \sheaf F$, so that by Proposition~\ref{prop:equivDeligneKashiwara}\ref{li:prop:equivDeligneKashiwara:le} we need to show that
    \[
        \dim \supp H^{k}\left(\sheaf G \otimes_{\sheaf{O}_X} \dualize \sheaf F\right) \le - k - n 
    \]
    for all $k$.
