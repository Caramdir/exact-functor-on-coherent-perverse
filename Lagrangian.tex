...
For an $\O_X$-module $\sheaf F$ let $\lc Z\sheaf F$ be the subsheaf of $\sheaf F$ of sections with support in $Z$ \cite[Varition~3 in IV.1]{Hartshorne:1966:ResiduesAndDuality}.
...
The topological motivation was explained to me by David Nadler.
I would also like to thank Sam Gunningham for suggesting improvements. 
...
\begin{proof}\leavevmode
    \begin{enumerate}[(a)]
        ...
        \item
            ...
            If $\dim Y = 0$, then $\mathbf ι_Y^!\sheaf F = Γ(X,\lc Y F)$ and thus $\lc Y\sheaf F ∈ D^{≥p(0)}(X)$ by assumption.
...
\begin{Thm}
    ...
    Then,
    \begin{enumerate}
        \item $\perv[p] D^{≤0}(X) = \left\{ \sheaf F ∈ D_c^b(X_G) : \lc Z\sheaf F ∈ D^{≤0}(X) \text{ for all $p$-measuring subvarieties $Z$}\right\}$;
        \item $\perv[p] D^{≥0}(X) = \left\{ \sheaf F ∈ D_c^b(X_G) : \lc Z\sheaf F ∈ D^{≥0}(X) \text{ for all $p$-measuring subvarieties $Z$}\right\}$.
    \end{enumerate}
...
\begin{Lem}
    \label{lem:constructible_local_vanishing}%
    Let $\sheaf F$ be a constructible sheaf on a real manifold concentrated in degree $0$ and let $i\colon Z \hookrightarrow X$ be the inclusion of a closed submanifold.
    Then $H^j(i^!\sheaf F) = 0$ for $j>\codim_XZ$.
\end{Lem}

\begin{proof}[Proof]\hspace{-1.3ex}{\it \footnotemark}\footnotetext{Following \url{http://mathoverflow.net/questions/129244}.}
    ...
    \]
    By \cite[?]{KashiwaraSchapira} we have
    \[
...
\begin{Rem}
    The equality $\perv D^{≥0}(X) = \perv[L]D^{≥0}(X)$ could also be proved in exactly the same way as in the coherent case, using \cite[Exercise~X.10]{KashiwaraSchapira:1994:SheavesOnManifolds}.
\end{Rem}
