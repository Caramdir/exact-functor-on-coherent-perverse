...
\title{Perverse coherent sheaves and measuring subvarieties}
...
\begin{abstract}
    Inspired by microlocal characterizations of constructible perverse sheaves we consider an alternative definition of perverse coherent sheaves as the category consisting of coherent sheaves $\sheaf F ∈ D^b_c(X^G)$ for which $RΓ_Z\sheaf F$ is concentrated in degree $0$ for special subvarieties $Z$ of $X$.
\end{abstract}
...
Consider a complex Kähler manifold $X$ and a perverse (constructible) sheaf $\sheaf F$ on $X$ with respect to the middle perversity.
If $f\colon X → ℂ$ is a holomorphic function, then the vanishing cycles $φ_f(\sheaf F)$ is again perverse.
Assume in addition that $f$ has a single critical point $p$ with $f(p) = 0$ and that the Hessian of $f$ is non-degenerate at $p$.
...
This observation can be used as a criterion for perversity of a constructible sheaf.
The question arises, whether an analogous criterion can be given for perverse \emph{coherent} sheaves (see Section~\ref{sec:Kashiwara} for a review of the theory of perverse coherent sheaves).
To do so we start from the Lagrangian $C^+_{f,p}$, rather than from the function $f$.
The local cohomology along such a Lagrangian should give information about whether a sheaf is perverse.

In the coherent setting, in order to have a middle perversity, we need to take a variety $X$ with an action of a group $G$ such that all orbits are even dimensional.
As an analogue of Lagrangian submanifolds, we define \enquote{measuring subvarieties} (Definition~\ref{def:measuring}), which are essentially closed subvarieties that intersect each $G$-orbit $U$ in half the dimension of $U$.
Our main theorem (Theorem~\ref{thm:main}) then states that a coherent sheaf $\sheaf F ∈ D^b_c(X^G)$ is perverse if and only if $RΓ_Z(\sheaf F)$ is concentrated in degree $0$ for all measuring subvarieties $Z$ of $X$.
Thus it is possible to define the category of perverse coherent sheaves as an Abelian subcategory of $D^b_c(X^G)$ directly by the single type of measurement $RΓ_Z$, instead of having to define a t-structure first.
...
The following two lemmas are central for the proof of the two respective parts of the theorem.
