...
We prove the following analogue of Theorem~\ref{thm:main}.

\begin{Thm}
    A sheaf $\sheaf F ∈ D^b_{\setset S}(X)$ is perverse if and only if the following condition holds:
    \begin{quote}
        For each real submanifold $Y$ of $X$ such that $\dim_ℝ(S ∩ Y) = \frac 12 \dim_ℝ S$ for each stratum $S ∈ \setset S$, the sheaf $\mathbf ι_Y^! \sheaf F$ is concentrated in cohomological degree $0$.
    \end{quote}
\end{Thm}

\begin{proof}
    First show that there are enough such submanifolds.
    ...
    In general, let $U$ be the open stratum and $F$ its complement.
    By induction, the theorem holds on $U$ and $F$.
    By recollement it suffices to show the conditions holds for a sheaf $F$ on $X$ if and only if it holds of $ι_U^*$, \ldots
\end{proof}
