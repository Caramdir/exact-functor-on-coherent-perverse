...
\begin{abstract}
    ...
    we show that a coherent sheaf is perverse if and only if $RΓ_Z\sheaf F$ is concentrated in degree $0$ for special subvarieties $Z$ of $X$.
...
All functors are derived, though we usually don't explicitly mention it in the notation.
...
\begin{proof}\leavevmode
    \begin{enumerate}[(a)]
        ...
        \item
            ...
            \]
            Now, $k-1 \le p(x) - 2$, so that $H^{k-1}(j_*j^*\sheaf F)$ is coherent by the Grothendieck finiteness theorem in the form of \cite[Corollary~3]{Bezrukavnikov:arXiv:PerverseCoherentSheaves}.
            ...
            Clearly $\lc Z τ^{>k}\lc {Z}\sheaf F$ is also in degrees larger than $k$.
